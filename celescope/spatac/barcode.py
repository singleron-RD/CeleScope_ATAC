...
class Barcode(super_barcode.Barcode):
    ...
    def run(self):
        ...
        for i in range(self.fq_number):
            ...
            ) as fq1, pysam.FastxFile(
                self.fq2_list[i], persist=False
            ) as fq2, pysam.FastxFile(self.fq3_list[i], persist=False) as fq3:
...
def get_opts_barcode(parser, sub_program=True):
    ...
    parser.add_argument(
        "--removed_barcodes",
        help="removed barcodes in valid reads.",
        default="GGTGGGTGGGTG",
    )
    if sub_program:
        parser.add_argument(
            "--fq3",
            help="R3 fastq file. Multiple files are separated by comma.",
            required=True,
        )
