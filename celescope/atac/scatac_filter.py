...
def Filter(rawmatrix, feature, barcode, peak_cutoff, cell_cutoff, outprefix):
    ...
    write_10X_h5(outprefix + "_filtered_peak_count.h5", matrix = passed_cell_matrix, features = passed_peaks, barcodes = passed_barcodes, datatype = 'Peak')
...
def scatac_qc(outprefix, peakcount, peak_cutoff, cell_cutoff):
    ...
    Filter(rawmatrix = peakmatrix, feature = features, barcode = barcodes, peak_cutoff = peak_cutoff, cell_cutoff = cell_cutoff, outprefix = filename)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="filter peak count matrix")
    parser.add_argument("--peakcount", help="Location of peak count matrix file", required=True)
    parser.add_argument("--outprefix", help="Prefix of output files", required=True)
    parser.add_argument("--peak_cutoff", type=int, help="Minimum number of peaks included in each cell", required=True)
    parser.add_argument("--cell_cutoff", type=int, help="Minimum number of cells covered by each peak", required=True)
