    ...
    for i in range(0,len(count_list)):
        ...
        for line in open(count_list[i], 'r'):
            ...
            if line[0]+':'+line[1]+'-'+line[2] in peak_count:
                peak_count[line[0]+':'+line[1]+'-'+line[2]][0, i] = int(line[-1])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--barcode", help="Location of valid cell barcode file", required=True)
    parser.add_argument("--count_cutoff", help="Cutoff for the number of count in each cell", required=True)
    ...
    parser.add_argument("--fragment", help="Location of fragments.tsv file", required=True)
    parser.add_argument("--outprefix", help="Prefix of output files", required=True)
    args = parser.parse_args()
    peakcount(args.barcode, args.count_cutoff, args.cores, args.peak, args.fragment, args.outprefix)