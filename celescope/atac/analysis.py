...
class Analysis(Step):
    ...
    def __init__(self, args, display_title=None):
        ...
        self.fragment_files = f"{self.analysis_dir}/fragments_corrected_dedup_count.tsv.gz*"
    ...
    def run_snapatac2(self):
        ...
        genome = snap.genome.Genome(
            fasta=f'{self.reference}/genome.fa',
            annotation=f'{self.reference}/gene.gtf'
        )
    ...
    def cp_files(self):
        """copy files"""
        files = [self.df_tsne_file, self.peak_res, self.cell_qc_metrics, f"{self.analysis_dir}/peak/*.h5", self.fragment_files]
        for file in files:
